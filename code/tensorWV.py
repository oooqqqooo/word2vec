...
def read_data():
    ...
    output = open('word.txt','w',encoding='utf-8')
    f = open('text_oneweek.txt',encoding='utf-8')
    index = 0;raw_word_list = []
    ...
    for i in f.readlines():
        ...
        index = index + 1
            ...
            while ' ' in line:    #去除空格
                line = line.replace(' ', '')
            while len(emoji1.findall(line)) >= 1 or len(emoji2.findall(line)) >= 1:            #去除emoji表情
                try:
                    emoji2.sub(line,'')
                    emoji1.sub(line,'')
                except re.error:
...
#验证集
valid_word = ['支付宝','贷款','理财','金融','周星驰','可爱',"跑步","超级","吃饭"]
...
try:
    ...
    font = FontProperties(fname=r"/usr/local/lib/python3.5/dist-packages/matplotlib/mpl-data/fonts/ttf/SimHei.ttf", size=14)
...
with open('embeddingsChinese.txt','w',encoding = 'utf-8') as file:
        for i in range(len(final_embeddings)):
            word = reverse_dictionary[i]
            vector = []
            for j in range(len(final_embeddings[i])):
                vector.append(final_embeddings[i][j])
